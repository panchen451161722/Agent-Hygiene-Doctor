...
describe("setup destinations", () => {
  it("uses agent-specific directories and explicit environment overrides", async () => {
    const context = await createScanContext({ platform: "linux", paths: posixDialect, selectedWorkingDirectory: "/repo", projectRoot: "/repo", environment: { HOME: "/home/a", CLAUDE_CONFIG_DIR: "/cfg", HERMES_HOME: "/hermes" }, fs: { lstat: async () => "directory" as const }, toolVersion: "1.1.0" });
