...
describe("setup destinations", () => {
  it("uses agent-specific directories and explicit environment overrides", async () => {
    const context = await createScanContext({ platform: "linux", paths: posixDialect, selectedWorkingDirectory: "/repo", projectRoot: "/repo", environment: { HOME: "/home/a", CLAUDE_CONFIG_DIR: "/cfg", HERMES_HOME: "/hermes", PI_CODING_AGENT_DIR: "/pi-agent" }, fs: { lstat: async () => "directory" as const }, toolVersion: "1.1.0" });
    ...
    expect(resolveSetupDestination("pi", context).skillPath).toBe("/pi-agent/skills/agent-hygiene/SKILL.md");
  });
