...
export const scanLocal = async (options: ScanOptions): Promise<{ readonly report: ReportV1; readonly projectDirectory: string }> => {
  ...
  const findings = [...analyzeConfiguration(inventory), ...analyzeMcp(inventory), ...analyzeSkills(inventory)];
  return {
    projectDirectory,
    report: buildReport({
