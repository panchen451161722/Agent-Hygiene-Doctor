...
export const installOwnedFiles = async (fs: OwnedFileSystem, files: readonly { readonly path: string; readonly content: string }[], manifestPath: string): Promise<SetupManifest> => {
  for (const file of files) {
    try {
      ...
      if (error instanceof Error && error.message === "AH-SETUP-CONFLICT") throw error;
    }
  }
  for (const file of files) {
...
export const uninstallOwnedFiles = async (fs: OwnedFileSystem, manifest: SetupManifest): Promise<void> => {
